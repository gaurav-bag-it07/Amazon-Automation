...
// let fid = " soraxot967@684hh.com";
// let fpass = "Qwerty@123";
let type = "Self";
...
browserPromise.then(function(browser){
    ...
    .then(function(pages){
        ...
        let secondprodagainPromise = tab.goto("https://www.amazon.in/Introducing-Echo-Show-8-display-screen/dp/B07SMNPCGK/ref=sr_1_1?dchild=1&keywords=amazon+echo+show&qid=1618118797&sr=8-1");
        return secondprodagainPromise;
    })
    .then(function(){
        let waitfbPromise = tab.waitForSelector("#add-to-cart-button",{visible:true});
        return waitfbPromise;
    })
    .then(function(pages){
        
        let shareonfbPromise = tab.goto("https://web.whatsapp.com/");
        return shareonfbPromise;
    })
    // .then(function(){
    //     let idfbPromise = tab.type("#email",fid);
    //     return idfbPromise;
    // })
    // .then(function(){
    //     let passfbPromise = tab.type("#pass",fpass);
    //     return passfbPromise;
    // })
    // .then(function(){
    //     let loginfbPromise = tab.click(".uiButton.uiButtonConfirm.uiButtonLarge");
    //     return loginfbPromise;
    // })
    .then(function(){
        let waitbeforelaunchPromise = tab.waitForSelector("chevron-right-text",{visible:true});
        return waitbeforelaunchPromise;
    })
    
    .then(function(){
        let selfsearchPromise = tab.type("._2_1wd.copyable-text.selectable-text",type);
        return selfsearchPromise;
    })
    .then(function(){
            let clickchatPromise = tab.click("._2Z4DV._2GVnY");
            return clickchatPromise;
        })
